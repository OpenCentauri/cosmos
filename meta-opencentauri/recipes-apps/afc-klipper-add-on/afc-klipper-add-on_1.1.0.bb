...
do_install() {
    ...
    #     cutter. Cutter is mounted on the right side of the toolhead and
    #     is engaged by pressing the toolhead in -Y against the front
    #     case. Measured on hardware: lever just touches at Y=18, and
    #     pushing past Y=2 / X=255 skips steps, so cap the cut endpoint
    #     at (X=254, Y=2). pin_loc_y=18, cut_move_dist=16 → full_cut_y =
    #     pin_loc_y - cut_move_dist = 2. pin_park_dist stays at the
    #     default 6 (toolhead parks at Y=24 before approaching).
    sed -i \
        ...
        -e 's|^variable_pin_loc_xy *: *-99, *-99|variable_pin_loc_xy               : 254, 18|' \
        -e 's|^variable_cut_direction *: *"left"|variable_cut_direction            : "front"|' \
        -e 's|^variable_cut_move_dist *: *8\.5|variable_cut_move_dist            : 16|' \
