...
# mid-chain failure: reset write error (PR #312's actual race)
setup
ln -sfn /dev/full "$CASE/sys/block/zram0/reset"
run_init start >/dev/null && { echo "FAIL: failing reset write should not succeed"; FAILED=1; }
check "gives up after 5 attempts on reset failure" grep -q "after 5 attempts" "$CASE/log"
check "no swap left active after reset failure" sh -c "! grep -q \"$CASE/zram0\" \"$CASE/proc/swaps\""
check "logs failure reason" grep -Eq "attempt 1 failed: .+" "$CASE/log"
